...
class Controller {
  output = {
    getMain: async (req, res) => {
      const params = { diff_date: 0, typeStatus: false, type: false };
    ...
    getMainSchedule: async (req, res) => {
      if (req.user) {
        ...
        const params = { diff_date: diff_date, typeStatus: false, type: false };
        const meeting = new Meeting(params);
        const meetingResult = await meeting.getScheduleInfo();
    ...
    createInquiry: async (req, res) => {
      if (req.user) {
        if (!req.body.title) return res.send(baseResponse.TITLE_EMPTY);
          ...
          const params = { userId: userId, title: title, content: content };
